...
manifest = json.loads((ROOT / "runtime" / "shuanglv-legal-skills" / "manifest.json").read_text(encoding="utf-8"))
...
stage_text = stage_map.get(manifest.get("release_stage", ""), "正式版" if manifest.get("release_status") == "FINAL_RELEASE" else manifest.get("release_stage", "公开共创版"))
stage_badge = urllib.parse.quote(stage_text, safe="").replace("-", "--").replace("_", "__")

def render(name: str) -> str:
    t = (TPL / name).read_text(encoding="utf-8")
    return t.replace("{{VERSION}}", display_version).replace("{{STAGE_BADGE}}", stage_badge)
...
if not re.search(r"(?m)^## 一、这是什么\s*$", body):
    sys.exit("README.md 作者正文缺少‘一、这是什么’起始章节，请检查")
